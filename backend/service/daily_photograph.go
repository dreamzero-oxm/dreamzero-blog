...
type CreateDailyPhotographService struct {
	...
	Photos      []*multipart.FileHeader `form:"photos"`      // 照片文件列表，必填
	Title       string                  `form:"title"`       // 照片标题
	Description string                  `form:"description"` // 照片描述
	// TODO: 改成[]string
	Tags         string  `form:"tags"`          // 照片标签，多个标签用逗号分隔
	TakenAt      string  `form:"taken_at"`      // 拍摄时间，格式为"2006-01-02"
	Location     string  `form:"location"`      // 拍摄地点
	Camera       string  `form:"camera"`        // 相机型号
	Lens         string  `form:"lens"`          // 镜头型号
	ISO          float64 `form:"iso"`           // ISO值
	Aperture     float64 `form:"aperture"`      // 光圈值
	ShutterSpeed float64 `form:"shutter_speed"` // 快门速度
	FocalLength  float64 `form:"focal_length"`  // 焦距
	IsPublic     bool    `form:"is_public"`     // 是否公开
}
...
type GetDailyPhotographsByDateRangeService struct {
	UserID    string `uri:"user_id" binding:"required"`     // 用户ID，必填
	StartDate string `form:"start_date" binding:"required"` // 开始日期，格式：2006-01-02
	EndDate   string `form:"end_date" binding:"required"`   // 结束日期，格式：2006-01-02
	Page      int    `form:"page" json:"page"`              // 页码，默认为1
	Size      int    `form:"size" json:"size"`              // 每页数量，默认为10
}
...
type UpdateDailyPhotographService struct {
	PhotoID      string    `form:"photo_id" binding:"required"` // 照片ID，必填
	UserID       uuid.UUID // 用户ID，必填，用于权限验证
	Title        string    `form:"title"`         // 照片标题
	Description  string    `form:"description"`   // 照片描述
	Tags         string    `form:"tags"`          // 照片标签，多个标签用逗号分隔
	TakenAt      string    `form:"taken_at"`      // 拍摄时间
	Location     string    `form:"location"`      // 拍摄地点
	Camera       string    `form:"camera"`        // 相机型号
	Lens         string    `form:"lens"`          // 镜头型号
	ISO          float64   `form:"iso"`           // ISO值
